        ...
        {guests.length === 0 ? (
          ...
          <>
                ...
                <tbody className="divide-y divide-gray-100">
                  {guests.map((guest) => (
                    <tr key={guest.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-5 py-3 text-gray-500 text-xs">{new Date(guest.created_at).toLocaleDateString()}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
            ...
            <div className="md:hidden divide-y divide-gray-100">
              {guests.map((guest) => (
                <div key={guest.id} className="p-4 space-y-1">
                  <div className="flex items-center justify-between">
                    <p className="font-medium text-gray-900 text-sm">{guest.attendee_name}</p>
                    {guest.checked_in ? (
                      <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded">Checked In</span>
                    ) : (
                      <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded">Registered</span>
                  ...
                  <div className="flex items-center gap-3 text-xs text-gray-400">
                    <span>{guest.tier_name}</span>
                    <span className="font-mono">{guest.confirmation_code}</span>
                  </div>
                </div>
              ))}
            </div>
